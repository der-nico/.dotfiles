...
# Function to search for pattern with rg choosing with fzf and then opening the corresponding file in vim
function virg () { vim "$(rg $* --line-number --color always | fzf --ansi --height 20% | sed 's/\(^[^:]*:[0-9]*:\).*/\1/')"}
alias vrg="virg"
...
if [ ${HOST:0:6} = "lxplus" ] || [ ${HOST:0:4} = "pc20" ]; then
  ...
  if [ ${HOST:0:6} = "lxplus" ]; then
    ...
    alias ktmux2="echo $HOST >>$HOME/tmux-sessions; pagsh.krb -c 'k5reauth -f -i 3600 -p nscharmb -k ~/private/tools/nscharmb.keytab && tmux'"
    # ktmux(){
    # if [[ -z "$1" ]]; then #if no argument passed
    #     echo $HOST >>$HOME/tmux-sessions;
    #     pagsh.krb -c 'kinit -k -t ~/private/tools/nscharmb.keytab nscharmb && k5reauth -f -i 3600 -p nscharmb -k ~/private/tools/nscharmb.keytab -- tmux new-session'
    # else #pass the argument as the tmux session name
    #     k5reauth -f -i 3600 -p nscharmb -k ~/private/tools/nscharmb.keytab -- tmux new-session -s $1
    # fi
  # }
